...
const CharacterForm = ({ onSuccess, initialData = {} }) => {
    ...
    const getSubclassOptions = (className) => {
        if (!className || !allSubclassFeatures[className]) return [];
        return Object.keys(allSubclassFeatures[className]);
    };
    ...
    const handleClassChange = (index, field, value) => {
        setForm(prevForm => {
            const updatedClasses = [...prevForm.classes];
        ...
        setClassFeatures(prevFeatures => {
            const updatedFeatures = [...prevFeatures];
            const currentClass = form.classes[index] || {};
            const cls = { ...currentClass, [field]: field === 'level' ? Number(value) : value };
            if (field === 'name') {
                cls.subclass = '';
            }
            ...
            let unlocked = [];
            let upcoming = [];
            if (cls.name && allClassFeatures[cls.name]) {
                unlocked = allClassFeatures[cls.name].features.filter(f => f.level <= cls.level);
                upcoming = allClassFeatures[cls.name].features.filter(f => f.level > cls.level);
            }

            let subclassUnlocked = [];
            let subclassUpcoming = [];
            if (cls.subclass && allSubclassFeatures[cls.name]?.[cls.subclass]) {
                const subclassFeaturesRaw = allSubclassFeatures[cls.name][cls.subclass];
                const subclassFeatures = toArray(subclassFeaturesRaw);
    ...
    const handleSubclassChange = (index, subclassName) => {
        ...
        setClassFeatures(prevFeatures => {
            const updatedFeatures = [...prevFeatures];
            const cls = { ...form.classes[index], subclass: subclassName };
            const level = cls.level || 1;

            const subclassFeaturesRaw = allSubclassFeatures[cls.name]?.[subclassName];
            const subclassFeatures = toArray(subclassFeaturesRaw);

            const subclassUnlocked = subclassFeatures.filter(f => f.level <= level);
            const subclassUpcoming = subclassFeatures.filter(f => f.level > level);
    ...
    const subclassOptions = form.classes.map(cls => getSubclassOptions(cls.name));
